...
void VkDescriptor::Init(VkDevice device, uint32_t maxSets, std::span<PoolSizeRatio> poolRatios)
{
	ratios.assign(poolRatios.begin(), poolRatios.end());

	VkDescriptorPool newPool = CreatePool(device, maxSets, poolRatios);

	setsPerPool = static_cast<uint32_t>(maxSets * 1.5); // Grow it for the next allocation
