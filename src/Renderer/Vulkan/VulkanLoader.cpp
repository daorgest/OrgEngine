...
std::optional<std::vector<std::shared_ptr<MeshAsset>>> VkLoader::LoadGltfMeshes(VkEngine* engine,
                                                                               ...
                                                                               bool OverrideColors, bool flipZAxis)
{
    ...
    // Parse the GLTF binary file
    auto loadResult = parser.loadGltfBinary(gltfFile.get(), filePath.parent_path(), gltfOptions);
    if (!loadResult)
    {
        LOG(ERR, "Failed to parse GLTF at path: ", filePath.string(), " Error code: ",
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes)
    {
        ...
        // Optionally override colors with normals for testing/debugging
        if (OverrideColors)
        {
        ...
        // Upload mesh data to Vulkan engine
