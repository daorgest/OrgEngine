/*
    ...
    WGL loader generated by glad 0.1.36 on Sat Sep 28 22:06:54 2024.
    ...
    Commandline:
        --api="wgl=1.0" --generator="c" --spec="wgl" --extensions="WGL_ARB_create_context,WGL_ARB_create_context_profile,WGL_ARB_create_context_robustness,WGL_ARB_extensions_string,WGL_ARB_framebuffer_sRGB,WGL_ARB_multisample,WGL_ARB_pixel_format,WGL_EXT_extensions_string,WGL_EXT_framebuffer_sRGB,WGL_EXT_multisample,WGL_EXT_swap_control,WGL_EXT_swap_control_tear,WGL_NV_gpu_affinity"
    Online:
        https://glad.dav1d.de/#language=c&specification=wgl&loader=on&api=wgl%3D1.0&extensions=WGL_ARB_create_context&extensions=WGL_ARB_create_context_profile&extensions=WGL_ARB_create_context_robustness&extensions=WGL_ARB_extensions_string&extensions=WGL_ARB_framebuffer_sRGB&extensions=WGL_ARB_multisample&extensions=WGL_ARB_pixel_format&extensions=WGL_EXT_extensions_string&extensions=WGL_EXT_framebuffer_sRGB&extensions=WGL_EXT_multisample&extensions=WGL_EXT_swap_control&extensions=WGL_EXT_swap_control_tear&extensions=WGL_NV_gpu_affinity
*/
...
int gladLoadWGLLoader(GLADloadproc load, HDC hdc) {
	// Load the function pointers for WGL extension strings
	glad_wglGetExtensionsStringARB = (PFNWGLGETEXTENSIONSSTRINGARBPROC)load("wglGetExtensionsStringARB");
	glad_wglGetExtensionsStringEXT = (PFNWGLGETEXTENSIONSSTRINGEXTPROC)load("wglGetExtensionsStringEXT");

	// Check if both extension string functions failed to load
	if (glad_wglGetExtensionsStringARB == NULL && glad_wglGetExtensionsStringEXT == NULL) {
		return 0;  // Failed to load the essential WGL extension functions
	}

	// Set the device context for core WGL functions
	find_coreWGL(hdc);

	// Find supported extensions for WGL
	if (!find_extensionsWGL()) {
		return 0;
	}

	// Load WGL extensions: context creation, pixel format, etc.
